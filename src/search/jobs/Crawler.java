...
public class Crawler {
                        ...
                        if (!kvs.existsRow("pt-crawl", Hasher.hash(url))) {
                            ...
                            if (crawlAllowed) {
                                ...
                                if (delayValid) {
                                    ...
                                    // if head results and get results do not have exception errors, return the new
                                    // URLs
    ...
    public static String[] sendHEAD(KVSClient kvs, String url) {
        ...
        try {
            ...
            connection.setRequestProperty("User-Agent", "search-crawler");
            ...
            // }
            // }
            // logger.info(" step5===send HEAD: connection headers: " +
            // headersLowerCase.toString());
        ...
        try {
            ...
            if (contentType.contains("text/html") && headResponseCode.equals("200") && contentLength < 5000000) {
                ...
                if (responseCode == 200) {
                    ...
                    logger.info("    step6===send GET: have put html content to pt-crawl, byte array length: "
            ...
            // handle redirect
            else if (headResponseCode.equals("301") || headResponseCode.equals("302") || headResponseCode.equals("303")
                    || headResponseCode.equals("307") || headResponseCode.equals("308")) {
                String redirectURL = connection.getHeaderField("Location");
                if (redirectURL != null) {
                    String nextURL = Crawler.normalizeURL(url, redirectURL);
                    if (nextURL != null) {
                        newURLs.add(nextURL);
                        logger.info("    send GET: base url: " + url + ",  redirection, wait for another round"
                                + Crawler.normalizeURL(url, redirectURL) + "code: " + headResponseCode);
                    }
                }
                return newURLs;
            }
            ...
            // other response code, content type not valid, content length exceeding 10 mb
                ...
                logger.info("    send GET: not valid: response Code: " + headResponseCode + "Content Type: "
    ...
    public static Map<String, List<String>> checkRobotsText(KVSClient kvs, String url) {
        ...
        try {
            ...
            if (kvs.existsRow("pt-host", hostDomainHashedKey)) {
                ...
                String robotURL = URLParser.parseURL(url)[0] + "://" + URLParser.parseURL(url)[1] + ":"
                        + URLParser.parseURL(url)[2] + "/robots.txt";
    ...
    public static Map<String, List<String>> parseRobotsText(String text) {
        ...
        if (text.indexOf("user-agent: search-crawler\n") != -1) {
            int pos = text.indexOf("user-agent: search-crawler\n");
            text = text.substring(pos + "user-agent: search-crawler\n".length());
            ...
            if (kvs.existsRow("pt-host", hostDomainHashedKey)) {
                ...
                if (value != null) {
                    ...
                    if (System.currentTimeMillis() - timeStampLong > interval) { // more than 1 sec
                        kvs.put("pt-host", hostDomainHashedKey, "timeStamp",
                                String.valueOf(System.currentTimeMillis()).getBytes());
                        logger.info("    step4===check crawl delay: update time to"
                    ...
                    kvs.put("pt-host", hostDomainHashedKey, "timeStamp",
                            String.valueOf(System.currentTimeMillis()).getBytes());
                    logger.info("    step4===check crawl delay: create time stamp"
                            + String.valueOf(System.currentTimeMillis()));
                    return true;
                }
            } else {
        ...
        while (matcher.find()) {
            ...
            if (matcher2.find()) {
                ...
                if (matcher3.find()) {
                    ...
                    nextURL = nextURL.replace("\"", "").replace("\'", "").trim(); // remove the url quotation marks,
                                                                                  // white space
                    ...
                    // }catch (UnsupportedEncodingException e) {
                    // logger.warn("extracted url UnsupportedEncodingException error: nextURL: " +
                    // nextURL);
                    // }catch(java.lang.IllegalArgumentException e){
                    // logger.warn("extracted url IllegalArgumentException error: nextURL: " +
                    // nextURL);
                    //
                    // }
    ...
    // normalize seed url, add missing port number and truncate the #tags in the
    // path
    ...
    public static String normalizeURL(String baseURL, String url) {
        ...
        // relative path ,relative to the base url. will return null if the relative
        // path is wrong
