...
public interface SparkPairRDD {
  ...
  // in the current PairRDD, where v is computed as follows: Let v_1,...,v_N be
  // the
  // values associated with k in the current PairRDD (in other words, the current
  // PairRDD contains (k,v_1),(k,v_2),...,(k,v_N)). Then the provided lambda
  // should
  // be invoked once for each v_i, with that v_i as the second argument. The first
  // invocation should use 'zeroElement' as its first argument, and each
  // subsequent
  ...
  // flatMapToPair() is analogous to flatMap(), except that the lambda returns
  // pairs
  ...
  // comma-separated lists of the values from the original RDD and from R,
  // respectively.
  // For instance, if the original RDD contains (fruit,apple) and (fruit,banana)
  // and
  // R contains (fruit,cherry), (fruit,date) and (fruit,fig), the result should
  // contain
  // a pair with key fruit and value [apple,banana],[cherry,date,fig]. This method
  // is
