...
public class FlameRDDImpl implements FlameRDD{
    ...
    public FlameRDD intersection(FlameRDD r, boolean persistent) throws Exception {
    	checkIfDestroyed();
        return (FlameRDD) context.invokeOperation(tableName, null, "intersection", ((FlameRDDImpl) r).getTable(), "rdd", persistent);
    }
	...
	public String fold(String zeroElement, TwoStringsToString lambda, boolean persistent) throws Exception {
		checkIfDestroyed();
		byte[] lambdaAsBytes = Serializer.objectToByteArray(lambda);
        String result = (String) context.invokeOperation(tableName, lambdaAsBytes, "fold", zeroElement, "rdd", persistent);
        return result;
	}
	...
	public FlamePairRDD flatMapToPair(StringToPairIterable lambda, boolean persistent) throws Exception {
		checkIfDestroyed();
		byte[] lambdaAsBytes = Serializer.objectToByteArray(lambda);
        return (FlamePairRDD) context.invokeOperation(tableName, lambdaAsBytes, "flatMapToPair", null, "rdd", persistent);
	}
